...
export default definePluginEntry({
  ...
  register(api) {
    ...
    // gets its own audit log file at <OPENCLAW_HOME>/agents/<agentId>/peer-contract-enforcer-audit.jsonl.
    const openclawHome = process.env.OPENCLAW_HOME ?? "C:/Users/Administrator/.openclaw";
    const auditLogger = createAuditLogger({
      resolveFilePath: (event) => {
        const agentId = event?.agentId ?? "unknown";
        // Defensive: agentId may contain path-traversal chars from a malicious payload;
        // sanitize to a safe directory name (alnum + dash + underscore).
        const safe = String(agentId).replace(/[^a-zA-Z0-9_-]/g, "_");
        return `${openclawHome}/agents/${safe}/peer-contract-enforcer-audit.jsonl`;
      },
    });
    ...
    api.on("session_start", async (event) => {
      ...
      if (!sk) return;
      // Day 6a followup (Mavis 2026-08-22 10:25, Kelsen #8 feedback): read correlation_id
      // from event (OpenClaw may propagate dispatch-level fields on the event). Fall
      // back to event.payload.correlation_id. This is then read by tool-guard and
      // message_sending hooks for audit log chain tracking.
      //
      // Day 6a+ (Mavis 2026-08-22 10:30, Kelsen #8 v2 feedback P0-2): if event doesn't
      // have correlationId (OpenClaw SDK doesn't propagate it), inherit from existing
      // sessionRegistry entry. This protects the pre-fill done in tool-guard's
      // sessions_send branch (which sets correlationId BEFORE the target session
      // actually starts, so session_start can inherit it).
    ...
    api.on("subagent_spawned", async (event) => {
      ...
      if (!childKey) return;
      // Day 6a followup: inherit correlation_id from parent session if not explicit.
      const parentSk = event.sessionKey;
      const parentEntry = parentSk ? sessionRegistry.get(parentSk) : undefined;
      const correlationId = event.correlationId
        ?? event.correlation_id
        ?? event.payload?.correlation_id
        ?? parentEntry?.correlationId;
    ...
    api.on("message_sending", async (event) => {
      ...
      // Day 6a followup (Mavis 2026-08-22 10:25, Kelsen #8 feedback): resolve correlation_id
      // from event direct, then ctx, then sender session's stored correlationId.
      const senderSk = event.ctx?.sessionKey;
      const senderEntry = senderSk ? sessionRegistry.get(senderSk) : undefined;
      const correlationId = event.correlationId
        ?? event.correlation_id
        ?? event.ctx?.correlationId
        ?? senderEntry?.correlationId;
